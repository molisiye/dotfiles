...
helpers.ccontainer = function(widget)
	...
	container:connect_signal("mouse::enter", function()
		container.bg = beautiful.mouse_enter .. "60"
	...
	container:connect_signal("button::press", function()
		container.bg = beautiful.mouse_press .. "60"
	end)
...
helpers.watch = function(script, time)
	return awful.widget.watch(script, time or 15, function(widget, stdout)
		widget.markup = stdout
		widget.font = "Roboto Mono Nerd Fonts Bold 9"
	...
	elseif action == "web_search" then
		awful.prompt.run({
			...
			exe_callback = function(input)
				...
				awful.spawn.with_shell("xdg-open https://duckduckgo.com/?q=" .. input)
				naughty.notify({
					title = "Searching the web for",
					text = input,
					icon = gears.color.recolor_image(icons.browser, ICON_COLORS[COLORS]),
					urgency = "low",
				})
			end,
		})
