...
local icon = wibox.container.background(
	helpers.imaker(icons.color_icon(icons.volume2, ICON_COLORS[COLORS]), dpi(30), dpi(30)),
...
local update_icon = function(volume_level)
	awful.spawn.easy_async_with_shell([[pactl get-sink-mute 0]], function(stdout)
		if string.match(stdout, "no") then
			if volume_level <= 50 and volume_level > 0 then
				icon.widget = helpers.imaker(icons.color_icon(icons.volume0, ICON_COLORS[COLORS]), dpi(30), dpi(30))
			elseif volume_level <= 100 and volume_level > 50 then
				icon.widget = helpers.imaker(icons.color_icon(icons.volume1, ICON_COLORS[COLORS]), dpi(30), dpi(30))
			elseif volume_level <= 150 and volume_level > 100 then
				icon.widget = helpers.imaker(icons.color_icon(icons.volume2, ICON_COLORS[COLORS]), dpi(30), dpi(30))
			end
		else
			icon.widget = helpers.imaker(icons.color_icon(icons.volumex, ICON_COLORS[COLORS]), dpi(30), dpi(30))
		end
	end)
...
local placement_placer = function()
	...
	if right_panel and left_panel then
		if right_panel.visible then
			awful.placement.bottom_left(volume_osd, {
				margins = {
					left = osd_margin,
					right = 0,
					top = 0,
	...
	awful.placement.bottom_right(volume_osd, {
		margins = {
			left = 0,
			right = osd_margin,
			top = 0,
