...
bri_osd_slider:connect_signal("property::value", function()
	...
	awful.spawn("brightnessctl set " .. math.max(brightness_level, 5) .. "%", false)
...
local icon = wibox.widget({
	{
		image = icons.color_icon(icons.sun, ICON_COLORS[COLORS]),
		resize = true,
...
local placement_placer = function()
	...
	if right_panel and left_panel then
		if right_panel.visible then
			awful.placement.bottom_left(brightness_osd, {
				margins = {
					left = osd_margin,
					right = 0,
					top = 0,
	...
	awful.placement.bottom_right(brightness_osd, {
		margins = {
			left = 0,
			right = osd_margin,
			top = 0,
