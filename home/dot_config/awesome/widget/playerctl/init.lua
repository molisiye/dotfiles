...
-- buttons
local previous = mat_icon_button_rect(mat_icon(icons.color_icon(icons.prev, ICON_COLORS[COLORS]), dpi(11)))
previous:buttons(gears.table.join(awful.button({}, 1, nil, function()
	playerctl:previous()
end)))

local next = mat_icon_button_rect(mat_icon(icons.color_icon(icons.next, ICON_COLORS[COLORS]), dpi(11)))
next:buttons(gears.table.join(awful.button({}, 1, nil, function()
	playerctl:next()
end)))

local play = mat_icon_button_rect(mat_icon(icons.color_icon(icons.ppause, ICON_COLORS[COLORS]), dpi(18)))
play:buttons(gears.table.join(awful.button({}, 1, nil, function()
...
local music_artist = wibox.widget({
	font = "Roboto Mono Nerd Fonts Bold 10",
	valign = "center",
	widget = wibox.widget.textbox,
})
...
playerctl:connect_signal("metadata", function(_, title, artist, album_path, album, new, player_name)
	...
	if new == true then
		naughty.notify({
			app_name = "Music",
			title = title,
			text = helpers.colorize_text(artist, beautiful.accent_normal),
			image = album_path,
		})
	end
